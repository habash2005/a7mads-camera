...
export default function AdminDashboard() {
  ...
  if (!user) { window.location.hash = "#/admin/login"; return null; }
  if (!isAdmin) {
    return (
      <section className="container-site py-12">
        <h2 className="text-xl font-semibold">Admins only</h2>
        <p className="text-sm text-[color:var(--muted)] mt-2">This account is not on the admin allow-list.</p>
      </section>
    );
  }
              ...
              {upcoming.rows.map((b) => (
                <tr key={b.id} className="border-t border-neutral-200/60">
                  ...
                  <td className="px-3 py-2 space-x-2">
                    <button onClick={() => changeStatus(b, "confirmed")} className="text-emerald-700 hover:underline">
                      Confirm
                    </button>
                    <button onClick={() => changeStatus(b, "finished")} className="text-gold hover:underline">
                      Finish
                    </button>
                    <button onClick={() => cancelBooking(b.id)} className="text-wine hover:underline">
                      Cancel
                    </button>
                    <button onClick={() => deleteBooking(b.id, b.status)} className="text-rose hover:underline">
                      Delete
                    </button>
                  </td>
                </tr>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {refIndex.rows.length === 0 ? (
                <tr><td className="px-3 py-8 text-center text-sm text-neutral-500" colSpan={5}>No bookings.</td></tr>
              ) : (
                filteredRefs.map((r) => (
