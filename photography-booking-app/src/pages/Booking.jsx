...
export default function Booking(){
  ...
  const reset = () => {
    setStep(0); setSelected(PACKAGES[0]); setDate(''); setTime(''); setAvailability(null)
    setDetails({ name:'', email:'', phone:'', location:'Studio' }); setSubmitting(false); setResult(null)
  }
        ...
        <div className="glass p-5 md:p-6 rounded-2xl border border-rose/30 shadow-[0_10px_30px_rgba(0,0,0,0.08)] bg-white/80">
          ...
          {step===1 && (
            <div>
              ...
              {availability===true && (
                <p className="mt-3 text-sm text-green-700">✅ Slot available. You can proceed.</p>
              )}
              {availability===false && (
                <p className="mt-3 text-sm text-red-700">❌ Sorry, that time is taken. Try a different time.</p>
              )}
          ...
          {step===2 && (
            <div>
              ...
              <div className="mt-6 flex justify-between">
                <button className="text-sm underline text-charcoal/70 hover:text-rose" onClick={()=>setStep(1)}>← Back</button>
                <button
                  onClick={()=>setStep(3)}
                  disabled={!canNext2}
                  className={`rounded-full px-5 py-3 text-sm font-semibold shadow-md transition-all ${
          ...
          {step===3 && (
            <div>
              ...
              <div className="mt-4 grid md:grid-cols-2 gap-6">
                <div className="p-4 rounded-xl border border-rose/30 bg-blush/20">
                  ...
                  <ul className="mt-2 text-sm text-charcoal/80 space-y-1">
                    <li>Package: <span className="font-medium text-rose">{selected.name}</span> — ${selected.price}</li>
                    <li>Date &amp; Time: <span className="font-medium">{date || '—'} {time || ''}</span></li>
              ...
              {!result ? (
                <div className="mt-6 flex justify-between">
                  <button className="text-sm underline text-charcoal/70 hover:text-rose" onClick={()=>setStep(2)}>← Back</button>
                  <button
                    onClick={confirm}
                    disabled={submitting}
