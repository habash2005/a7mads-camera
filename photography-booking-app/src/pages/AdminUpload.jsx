...
export default function AdminUpload() {
  ...
  /* load bookings (prefer startAt; fallback to createdAt) */
  useEffect(() => {
    (async () => {
      try {
        ...
        try {
          snap = await getDocs(
            query(collection(db, "bookings"), orderBy("startAt", "desc"), fbLimit(200))
          );
        } catch {
          snap = await getDocs(
            query(collection(db, "bookings"), orderBy("createdAt", "desc"), fbLimit(200))
          );
        }
  ...
  async function storagePreflight(currentMode, refCode) {
    ...
    try {
      ...
      // (optional) you can inspect tok.claims if needed
      void tok;
    } catch {
      // ignore token inspect errors
    }
  ...
  async function getOrCreatePortfolioDoc() {
    const qy = query(collection(db, "galleries"), where("tag", "==", "portfolio"), fbLimit(1));
  ...
  async function onUpload() {
    ...
    if (mode === "client" && !selectedBooking) {
      alert("Choose a client reference first.");
      return;
    }
      ...
      setQueue((q) =>
        q.map((it) =>
          it.status === "queued" || it.status === "uploading"
            ? { ...it, status: "error", error: `Preflight failed: ${e.message || String(e)}` }
            : it
        )
      );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:items-end">
        ...
        <div className="lg:col-span-5" ref={clientDropRef}>
          <label className="block text-sm font-medium text-charcoal/80 mb-1">
            {mode === "client"
              ? "Choose client by reference"
              : "Client (disabled in Portfolio mode)"}
          </label>
